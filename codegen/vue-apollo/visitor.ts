...
> {
  ...
  constructor(
    ...
    super(schema, fragments, rawConfig, {
      withCompositionFunctions: getConfigValue(
        rawConfig.withCompositionFunctions,
        true
      ),
  ...
  protected buildOperation(
    ...
    operationVariablesTypes =
      this.externalImportPrefix + operationVariablesTypes
    ...
    const suffix = this.getCompositionFunctionSuffix(
      node.name.value,
      operationType
    )
    ...
    const documentNodeVariable = this.getDocumentNodeVariable(
      node,
      documentVariableName
    ) // i.e. TestDocument
    ...
    switch (operationType) {
      case 'Query': {
        return `export function use${operationName}(${
          ...
          operationHasVariables ? 'variables' : 'undefined'
        }, options);
            }`
      }
      case 'Mutation': {
